...
1. **事前確認**
   ...
   - 執筆対象のエピソード番号を確認
   - **前後のエピソードファイル（`episodes/XXX.txt`）を必ず直接読み、文体、雰囲気、ストーリーの詳細な流れを確認する**
     - 前話（例: 017.txtを書く場合は016.txtを読む）
     - 可能であれば前々話も確認し、展開の流れを把握する
     - 梗概だけでなく、実際の文章の雰囲気や会話の調子も確認する
   - `episode.md` で全体の流れと各話の梗概を確認
   - `config/index.md` を参照して、関連する設定ファイルを読み込み、矛盾がないか確認
   - プロットファイルで今後の展開を確認
